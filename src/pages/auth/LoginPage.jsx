...
import { signin } from "../../features/authApi.js";
...
/**
 * Backend returns a cookie-like string:
 * "homeBuddy=<JWT>; Path=/api; Max-Age=...; Secure; HttpOnly"
 * We extract the token value after "homeBuddy="
 */
function extractToken(jwtTokenString) {
    if (!jwtTokenString || typeof jwtTokenString !== "string") return "";
    const match = jwtTokenString.match(/homeBuddy=([^;]+)/);
    return match?.[1] || "";
}
                ...
                <section className="flex items-center justify-center px-6 py-10 md:px-10 md:py-0">
                    <div className="w-full max-w-md md:h-[calc(100vh-64px)] md:flex md:flex-col md:justify-center">
                        <div className="relative rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60 md:max-h-[calc(100vh-80px)] md:overflow-hidden">
                            {/* scroll container (keep for symmetry with Signup) */}
                            <div className="relative p-6 md:p-8 md:max-h-[calc(100vh-80px)] md:overflow-auto scrollbar-hidden">
                                <p className="text-xs font-bold tracking-widest text-slate-500">WELCOME BACK</p>
                                <h2 className="mt-2 text-3xl font-extrabold text-slate-900">Log in</h2>
                                <p className="mt-2 text-slate-600">Pick up where you left off — calm, simple, secure.</p>
                                ...
                                <div className="mt-6">
                                    ...
                                    <form onSubmit={onSubmit} className="space-y-4 pb-5 md:pb-5">
                                        <Input
                                            label="Username"
                                            name="username"
                                        ...
                                        <div className="relative">
                                            ...
                                            <button
                                                ...
                                                className="absolute right-3 top-[42px] rounded-lg px-2 py-1 text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100"
                                            >
                                                {showPw ? "Hide" : "Show"}
                                            </button>
                                        </div>
                                        ...
                                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                                            <p className="font-semibold text-slate-900">Having trouble logging in?</p>
                                            <p className="mt-1">
